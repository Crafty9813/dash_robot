...
class DashRobotEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: DashRobotEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.dof_idx, _ = self.robot.find_joints(self.cfg.joint_names)
    ...
    def _apply_action(self) -> None:
        self.robot.set_joint_effort_target(self.actions * self.cfg.action_scale, joint_ids=self.joints)

    def _get_observations(self) -> dict:
        # Based on joint positions and velocities
        obs = torch.cat(
            (
                self.joint_pos[:, self.joints[0]].unsqueeze(dim=1),
                self.joint_vel[:, self.joints[0]].unsqueeze(dim=1),
            ),
            dim=-1,
        )
